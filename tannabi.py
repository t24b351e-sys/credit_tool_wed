...
# 学籍番号
student_id = st.text_input("学籍番号を入力")

# 必要単位 & 講義リストの読み込み
required = read_requirements(req_file)
courses = read_courses()

st.subheader("必要単位（選択モードに対応）")
st.write(required)
...
for cat, subject_list in courses.items():
    ...
    options = [f"{name}（{credit}）" for name, credit in subject_list]
    chosen = st.multiselect("選択", options, key=cat)
    # 表示名から元データへ戻す
    chosen_names = [c.split("（")[0] for c in chosen]
    earned_courses[cat] = [(name, credit) for name, credit in subject_list if name in chosen_names]
    ...
    for cat in ["A","B0","B1","C","D","E"]:
        ...
        if cat == "B1":
            st.write(
                ...
                f"（B0余剰+{overflow['surplus_b0']} → 実効{overflow['eff_b1']}）"
                f" / 残り{overflow['remain_b1']}"
            )
        else:
            remain = max(0, need - got)
